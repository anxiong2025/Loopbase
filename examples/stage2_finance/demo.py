"""金融领域 demo：通用内核 + 金融工具集（当前唯一 demo）。

直接对接内核 ReActLoop / OpenAICompatibleClient，只注册金融工具集
（finance_agent.tools），演示内核的领域无关性。每轮打印发出去的请求
...
跑法（先 uv sync 安装 finance-agent）：
    uv sync
    uv run examples/stage2_finance/demo.py
    uv run examples/stage2_finance/demo.py "分析一下特斯拉（TSLA）"
"""
...
def main() -> None:
    ...
    loop = ReActLoop(
        ...
        evidence_log=JsonlEvidenceLog(DEMO_DIR / "evidence_finance.jsonl"),
    )
    ...
    print(f"\n证据日志已写入：{(DEMO_DIR / 'evidence_finance.jsonl').resolve()}")
